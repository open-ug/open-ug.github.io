...
begin
  ...
  select
    ...
    count(*) filter (where u.email_confirmed_at is null)::bigint,
    ...
    count(*) filter (
      where exists (
        select 1 from public.applications a where a.applicant_id = u.id
      )
    )::bigint,
    count(*) filter (where ur.role = 'admin')::bigint,
    count(*) filter (where ur.role = 'reviewer')::bigint,
    count(*) filter (where u.banned_until > now())::bigint
  from auth.users u
...
begin
  ...
  with application_counts as (
    select
      ...
      count(*) filter (where status <> 'draft')::bigint as submitted
