...
export default async function ProgramCta({
  compact = false,
}: {
  compact?: boolean;
}) {
  ...
  const { data: program } = await supabase
    .from("programs")
    .select("status,applications_open_at,applications_close_at")
    .eq("slug", "makerere-cs-website")
    .maybeSingle();
  const now = Date.now();
  const open =
  ...
  const href = open
    ? "/programs/makerere-cs-website/apply"
    : "/programs/makerere-cs-website";
  if (compact)
    return (
      <Link
        href={href}
      ...
      >
        {open ? "Start application" : "View program status"}
        <ArrowRight size={17} />
      </Link>
    );
  return (
    <>
      ...
      <p className="mt-2 text-sm leading-6 text-slate-600">
        {program?.applications_close_at
          ? `Deadline: ${new Intl.DateTimeFormat("en-UG", { dateStyle: "long", timeStyle: "short", timeZone: "Africa/Kampala" }).format(new Date(program.applications_close_at))}`
          : "View the program record for current application dates."}
      </p>
      <Link
        href={href}
      ...
      >
        {open ? "Apply to join" : "View program"}
