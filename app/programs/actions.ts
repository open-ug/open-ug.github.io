...
export async function saveApplication(formData: FormData) {
  ...
  if (!application || application.status !== "draft")
    redirect("/dashboard?error=This application can no longer be edited.");
  ...
  for (const question of questions ?? []) {
    ...
    if (answer === undefined) {
      if (intent === "submit" && question.required)
        redirect(
          `/dashboard/applications/${application.id}?error=${encodeURIComponent("Complete all required questions before submitting.")}`,
        );
  ...
  if (questionIdsToDelete.length) {
    ...
    if (error)
      redirect(
        `/dashboard/applications/${application.id}?error=${encodeURIComponent("Your draft could not be saved.")}`,
      );
  }
  if (answersToUpsert.length) {
    ...
    if (error)
      redirect(
        `/dashboard/applications/${application.id}?error=${encodeURIComponent("An answer is invalid or could not be saved.")}`,
      );
  }
  ...
  if (intent === "submit") {
    ...
    if (error)
      redirect(
        `/dashboard/applications/${application.id}?error=${encodeURIComponent(error.message.includes("window") ? "The application deadline has passed." : "Complete all required questions and check your profile before submitting.")}`,
      );
  ...
  if (existing) {
    redirect(
      existing.status === "draft"
        ? `${returnPath}/apply`
        : `/dashboard/applications/${existing.id}`,
    );
  }
  ...
  if (error?.code === "23505") {
    ...
    if (raced)
      redirect(
        raced.status === "draft"
          ? `${returnPath}/apply`
          : `/dashboard/applications/${raced.id}`,
      );
  }
  redirect(
    `${returnPath}?error=${encodeURIComponent("Unable to start an application. Check that applications are still open.")}`,
  );
}
...
export async function withdrawApplication(formData: FormData) {
  ...
  if (error)
    redirect(
      `/dashboard/applications/${applicationId}?error=This application cannot be withdrawn.`,
    );
  revalidatePath("/dashboard");
